...
const DiabetesPrediction = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setShowForm(false); // Hide the form when results are shown
      }, 3000);
    } catch (error) {
  ...
  const handleClear = () => {
    setResult(null);
    setShowGif(false);
    setShowForm(true); // Show the form again when cleared
    setFormData({
      gender: '',
      age: 10,
      hypertension: '',
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white pt-24 pb-16">
        {showGif ? (
          ...
          <>
            {showForm && (
              <div className="flex flex-col items-center justify-center w-full mt-3 mb-5">
                ...
                <form onSubmit={handleSubmit} className="rounded-lg p-6 w-96 flex flex-col bg-black shadow-[20px_20px_70px_rgba(255,255,255,0.2)] border-t-2 border-l-2 border-gray-950">
            ...
            {result !== null && (
              <div className="mt-6 p-4 bg-white shadow-[8px_8px_20px_rgba(255,193,7,0.5)] rounded-lg flex flex-col relative w-60 mx-auto">
                <button
                  onClick={handleClear}
